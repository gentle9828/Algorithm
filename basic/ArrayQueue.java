...
public class ArrayQueue implements Queue {
    ...
    public ArrayQueue(int queueSize) {
        ...
        queueArr = new int[this.queueSize]; // 큐 배열 설정
    }
    ...
    public boolean isFull() {
        // rear 포인터가 큐의 마지막 인덱스에 동일한 경우 true 아닌 경우 false return
    ...
    public int dequeue() {
        if(isEmpty()) {
            ...
            // front 포인터는 삭제할 위치에 있는 상태이므로 다음과 같이 (front + 1) % size로 설정.
            // front + 1로 설정하면 front 포인터가 마지막 요소에 위치했을 경우.
    ...
    public void clear() {
        if(isEmpty()) {
            ...
            front = -1; // front 포인터 초기화
            rear = -1; // rear 포인터 초기화
            queueArr = new int[this.queueSize]; // 새로운 큐 배열 생성
